...
- **IMP-02: CORS policy on mutating endpoints.** Resolved for every mutating endpoint: everything wired into the auth system (auth, admin, dashboard, images, list, plants, sourdough, jeger, recipes, workout, pricing, contact, store, and since 2026-07-18 stocks, rocks, iliana-photos, vrata, music) correctly omits `Access-Control-Allow-Origin`. Still sending `*`: only the read-only public proxies (apod, otd, stats, tabs, tarok), which is defensible.
