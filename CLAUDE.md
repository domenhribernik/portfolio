...
- `node --test tests/` runs unit tests for DOM-free frontend logic modules (currently `views/admin/logic.js` and `views/flowers/logic.js`). When a view's script grows non-trivial decision logic, extract it into a `logic.js` module the page imports and test it here instead of leaving it tangled in DOM code. Name browser-imported modules `.js`, not `.mjs`: Apache serves `.mjs` without a MIME type and browsers block it.
