...
- `/opt/lampp/bin/php tests/download.test.php` runs integration tests for the YouTube rip endpoint (`app/proxys/download.php`). No network, no yt-dlp, no database: it boots the built-in server three times with a different `YTDLP_BIN` injected into the process environment (a missing path, `/bin/false`, and a generated shell stub) to cover the graceful 503, URL and request validation, and the full prepare/file/cleanup lifecycle, cleaning up everything it created.
