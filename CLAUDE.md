...
Current project directories: `views/about`, `views/account`, `views/admin`, `views/blog`, `views/botaniq`, `views/dnd`, `views/download`, `views/flowers`, `views/homepage`, `views/hub`, `views/iliana`, `views/ip`, `views/jeger`, `views/list`, `views/maze`, `views/music`, `views/nebo`, `views/on-this-day`, `views/parlour`, `views/pricing`, `views/quizz`, `views/recipes`, `views/rocks`, `views/slovenia`, `views/sourdough`, `views/spy`, `views/stocks`, `views/tarok`, `views/thesis`, `views/valentine`, `views/vrata`, `views/workout`. (`views/shopping` is only a redirect stub left behind when the shopping app was renamed to `views/list`; delete it once the old installed PWAs are gone.)

Note: `views/account`, `views/admin`, `views/download`, `views/hub`, `views/list`, `views/pricing`, `views/stocks`, `views/valentine`, and `views/vrata` are **unlisted private tools**. They are intentionally not registered in `components/project-data.js`, not linked from `index.html`, and not in the main navbar. Do not add them to any of those. (`views/vrata` is also a standalone PWA with its own `manifest.json` and service worker, backed by `app/proxys/vrata.php`. `views/account` is the sign-in/account page and `views/admin` the admin dashboard of the auth system, see "Authentication and Permissions" below. `views/hub` is the installable PWA app launcher: its manifest `scope` resolves to the site root, so tiles open other views chrome-free inside the standalone window; backed by `app/controllers/hub-controller.php` and the `hub_apps` table in `app/models/hub-model.sql`, tiles are managed from the `views/admin` dashboard, and visibility is per-user via the projects registry, with a tile whose project is NULL visible to any signed-in user. `views/download` is a YouTube to mp3/mp4 downloader backed by `app/proxys/download.php`: no auth by design, it shells out to yt-dlp, which must be installed on the host (install and update commands are in the proxy's header comment), and answers a clear 503 where the binary is absent, e.g. the shared prod host; its page logic lives in `views/download/logic.js`.)
...
- [components/projects-grid.js](components/projects-grid.js): `<projects-grid category="...">` web component; auto-renders one `<project-card>` for every `project-data.js` entry matching that `category`, newest first. Used by `views/about`; the homepage projects section uses `<projects-paper>` instead.
- [components/projects-paper/](components/projects-paper/): `<projects-paper>` renders the homepage projects section as one newspaper front page from the `project-data.js` registry: a mono dateline masthead, then Professional & Freelance as the top-stories band (newest entry as a wide splash story, the rest in ruled columns beside it) and Passion Projects as the inside-pages index (all briefs flowing through CSS multicol newspaper columns). Academic entries are deliberately NOT carried on the front page; the paper's foot line cross-references `views/about/#academic-projects`, where they render via `<projects-grid>`. Each story is one stretched-link tap target pointing at the entry's primary link (visitSite, else readMore, else code, else demo); remaining links render as small underlined cross-references. Hover is deliberately quiet: the headline underlines in the section accent (clay band, cobalt briefs), nothing moves. On phones (<768px) each section prints at most `MOBILE_VISIBLE` (5) stories, the rest folded behind a mobile-only "More stories" toggle; wider viewports always print everything. DOM-free decision logic (section grouping, link priority, splash/brief split, the phone fold) lives in `projects-paper/logic.js`, tested by `tests/projects-paper-logic.test.mjs`; styles in [views/homepage/kinetic.css](views/homepage/kinetic.css).
...
- [app/proxys/](app/proxys/): External API proxies (hide API keys from the client) and small server-side endpoints. Current: `apod-proxy.php` (NASA APOD), `otd-proxy.php` (On This Day), `stats-proxy.php` (codebase line/file counts, cached daily), `tabs-proxy.php` (Songsterr tab search for the music view; Songsterr has no CORS), `vrata.php` (backend for the private `vrata` tool), `tarok.php` and `flowers.php` (share-link stores for the tarok scorekeeper and the flowers bouquets: JSON files under `app/cache/<name>/`, pruned after 7 days, no auth since share links are public by design), and `download.php` (backend for the unlisted `views/download` ripper: shells out to yt-dlp/ffmpeg, media plus JSON sidecars under `app/cache/download/`, pruned after 3 hours; that dir must pre-exist world-writable because Apache's daemon user cannot create directories under `app/cache/`).
...
- `node --test tests/` runs unit tests for DOM-free frontend logic modules (currently `views/admin/logic.js`, `views/flowers/logic.js`, and `views/download/logic.js`). When a view's script grows non-trivial decision logic, extract it into a `logic.js` module the page imports and test it here instead of leaving it tangled in DOM code. Name browser-imported modules `.js`, not `.mjs`: Apache serves `.mjs` without a MIME type and browsers block it.
