...
| `compass-controller.test.php` | `requireAdmin()` on every branch (signed-in non-admin is 403 on **reads** too), check-in upsert, catch create/delete, workbook upserts | Applies `compass-model.sql` if absent; fixture check-ins live on 1999 days |
