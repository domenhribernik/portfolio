...
| `medication-controller.test.php` | `requireAdmin()` on every branch (a signed-in non-admin is 403 on **reads** too), medication CRUD with soft-delete reads, the shared validation rules as 422s, the idempotent `PUT ?resource=dose` toggle, slots outside a medication's schedule | Applies `medication-model.sql` if absent; dose rows cascade off the medications, so an id baseline on `medication_meds` cleans up both tables | `pricing-controller.test.php` | Public POST create, IP privacy (stores only `ip_hash`, never returned), admin-gated list and hard DELETE | id-baseline teardown |
