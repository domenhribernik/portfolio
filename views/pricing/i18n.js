// Language system for the pricing calculator. Dictionaries live in
// lang/<code>.json, named by the ISO 639-1 code the browser reports
// (navigator.language gives "sl" or "sl-SI" on a Slovenian system), so the
// code doubles as the file name. The helpers up top (pickLanguage, lookup,
// format) are DOM-free and tested in tests/pricing-i18n.test.mjs; only
// applyTranslations touches the document. Ported from views/nebo/i18n.js,
// which this stays identical to on purpose: same contract, same tests.

export const SUPPORTED = ['en', 'sl'];
export const FALLBACK = 'en';
export const STORAGE_KEY = 'pricing-lang';
...
// sets attributes (aria-labels, placeholders). A node whose translation must
// coexist with other markup (e.g. a price chip nested in a label) should wrap
// just the translatable text in its own data-i18n span rather than putting
// the attribute on the parent, since this replaces the whole textContent.
