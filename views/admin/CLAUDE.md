...
- The admin dashboard is split into five hash-routed tabs (`#users`, `#projects`, `#dashboard`, `#leads`, `#marketing`) so the page never grows into one long scroll; `script.js` is an ES module. (The `#dashboard` tab manages the **Dashboard launcher**'s tiles, `views/dashboard`, not this admin page itself; the name overlap is deliberate.)
...
- Dashboard-launcher tiles carry an `is_default` flag (checkbox in the tile composer, `· default` marker in the list): default tiles are seeded onto every NEW user's personal shelf at signup by `app/services/dashboard-shelf-service.php`. Which tiles a user actually sees is their own choice, made in the Dashboard launcher's picker; this admin page only defines the tile pool, per-tile audience (project), and the new-user defaults. (The launcher also lets each user reorder tiles and group them into personal folders; that per-user layout lives in `dashboard_folders` + `dashboard_user_apps` and is not managed here.)
- The Leads tab is not backed by admin-controller.php: it lists `pricing_quotes` rows (submissions from the unlisted `views/pricing` quote calculator) via `pricing-controller.php?all=1`, gated by `Auth::requireAdmin()` in that controller directly, mirroring the Dashboard-launcher tiles pattern (feature data stays in the feature's own controller; the admin page just calls it). A quote only reaches the site owner's inbox today if the visitor clicked "send" in the calculator (which opens a `mailto:`), so this tab exists to catch the ones who filled out the form but never sent, e.g. bounced before clicking. The `contacted` boolean is toggled via `PATCH pricing-controller.php?id=<id>`, and a lead is hard-deleted (clearing junk / test rows) via `DELETE pricing-controller.php?id=<id>`, both admin-gated. Each lead row also has a **Send promo** button that opens the owner's mail client pre-filled with a personalized promo (their package + estimate); there is no server-side email, the owner presses send. The controller stores only `ip_hash` (daily-salted sha256), never a raw IP, matching `contact_messages` / `store_waitlist`. Backend covered by [tests/pricing-controller.test.php](../../tests/pricing-controller.test.php).
- The **Marketing** tab (`#marketing`) is a promo composer for cold outreach: an editable EN/SL template that pitches the `/views/pricing` calculator plus the pricing link, with copy-to-clipboard and an "open blank email" mailto. The pure builder (`buildPromoMessage`, `promoMailtoHref`, `PRICING_URL`) lives in [logic.js](logic.js) and is unit-tested in [tests/admin-logic.test.mjs](../../tests/admin-logic.test.mjs); the per-lead **Send promo** button reuses the same builder with the lead's details. Since the pricing page is unlisted, a lead only finds it via a link the owner sends.
- DOM-free decision logic (tab routing, list filters, Dashboard-tile form payload, tile reorder plan) lives in [logic.js](logic.js), unit-tested by [tests/admin-logic.test.mjs](../../tests/admin-logic.test.mjs) via `node --test tests/`. Put new pure logic there, test-first, rather than inline in script.js. Use the `.js` extension for browser-imported modules, never `.mjs`: Apache has no MIME mapping for `.mjs` and module scripts get blocked (the root package.json's `"type": "module"` is what lets node parse these `.js` files as ESM).
...
- Dashboard shelf semantics (personal picks AND permission, dormant rows, seeding, the per-user folder/layout save) are pinned by [tests/dashboard-controller.test.php](../../tests/dashboard-controller.test.php); the all-users grant by [tests/admin-roles.test.php](../../tests/admin-roles.test.php). Run them after touching `dashboard-controller.php` or `admin-controller.php` roles (see the root CLAUDE.md "Testing" section).
...
Roles are granted from this dashboard (per user, or via the "All users" fan-out). Use this layer alone when the whole feature has one audience (everyone with access sees the same data). Precedents: `images-controller.php` (public GETs, role-gated writes), Dashboard-launcher tiles (`dashboard_apps.project_id` decides which tiles a user MAY pick; their own `dashboard_user_apps` rows decide which actually show).
