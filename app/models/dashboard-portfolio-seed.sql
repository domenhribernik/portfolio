...
-- auth projects registry (the admin dashboard's projects tab), keeps the
-- Dashboard launcher to the genuinely useful launcher apps only (no academic
-- or professional entries anywhere), marks the default shelf, and assigns
-- those defaults to every existing active user. Run manually in phpMyAdmin.
-- Safe to re-run, with two caveats: section 3 deletes any tile it prunes even
-- if it was re-added by hand in the dashboard, and the backfill at the bottom
-- re-adds default tiles a user has since removed from their shelf. Treat
-- re-runs as a deliberate reset, not a routine step.
--
-- Standalone on purpose: it starts with the same idempotent schema catch-up
-- statements as dashboard-model.sql, so it works on a database at any state
-- (fresh local scratch, pre-rework prod, or prod after an earlier version of
-- this file), in either order relative to dashboard-model.sql.
--
-- Design note: the Dashboard tiles for public passion views deliberately
-- carry project_id NULL even though the projects are now registered below. A
-- tile linked to a project is visible only to role holders, and a brand-new
-- user has no roles at signup, so linked default tiles would sit dormant on
-- every new shelf. NULL-project tiles are visible to any signed-in user,
-- which is what makes the seeded defaults appear for future users with no
-- role grants. The registry rows exist for the dashboard's projects tab and
-- for any future role-gated feature inside those views; only the genuinely
-- gated private tools (Lists, Vrata) keep a project_id on their tile.

-- ------------------------------------------------------------------
-- 1. One-time rename from the old "hub_*" tables (data survives). Skip this
--    line once the dashboard_* tables exist (fresh DBs and any already
--    migrated DB fall through to the idempotent section 2).
-- ------------------------------------------------------------------
RENAME TABLE hub_apps TO dashboard_apps, hub_user_apps TO dashboard_user_apps;

-- ------------------------------------------------------------------
-- 2. Schema catch-up (idempotent copies of dashboard-model.sql pieces, so
--    this file works on a fresh DB without dashboard-model.sql having run).
-- ------------------------------------------------------------------
...
-- ------------------------------------------------------------------
-- 4. Register ALL passion projects in the projects registry (keys match
...
-- ------------------------------------------------------------------
-- 5. Repair the stale shopping-era tile (Todo -> Lists)
-- ------------------------------------------------------------------

INSERT INTO projects (project_key, name) VALUES ('list', 'Lists')
ON DUPLICATE KEY UPDATE name = name;

-- Rename in place unless a 'Lists' tile already exists (dashboard-model.sql
-- may have seeded one first); the derived table dodges MySQL error 1093.
UPDATE dashboard_apps
   SET name = 'Lists',
       ...
       sort_order = 140
 WHERE name = 'Todo'
   AND NOT EXISTS (SELECT 1 FROM (SELECT name FROM dashboard_apps) other WHERE other.name = 'Lists');

-- No-op after a successful rename; removes the stale duplicate otherwise.
DELETE FROM dashboard_apps WHERE name = 'Todo';

-- ------------------------------------------------------------------
-- 6. Repair existing tiles (match by url: names may have drifted)
-- ------------------------------------------------------------------
...
UPDATE dashboard_apps SET project_id = NULL, sort_order = 10  WHERE url LIKE '/views/botaniq%';
UPDATE dashboard_apps SET project_id = NULL, sort_order = 20  WHERE url LIKE '/views/sourdough%';
UPDATE dashboard_apps SET sort_order = 30  WHERE url LIKE '/views/workout%';
UPDATE dashboard_apps SET sort_order = 40  WHERE url LIKE '/views/recipes%';
...
UPDATE dashboard_apps SET sort_order = 150 WHERE url LIKE '/views/vrata%';
UPDATE dashboard_apps SET sort_order = 160 WHERE url LIKE '/views/jeger%';

-- ------------------------------------------------------------------
-- 7. The useful launcher apps (icons and gradients from
...
WHERE NOT EXISTS (SELECT 1 FROM dashboard_apps h WHERE h.url LIKE '/views/recipes%')
...
WHERE NOT EXISTS (SELECT 1 FROM dashboard_apps h WHERE h.url LIKE '/views/tarok%')
ON DUPLICATE KEY UPDATE dashboard_apps.name = dashboard_apps.name;

INSERT INTO dashboard_apps (name, icon, gradient, url, sort_order, project_id)
SELECT 'Paper Flowers', 'fas fa-spa', 'linear-gradient(45deg, #b13a6e 0%, #f6c1d9 100%)', '/views/flowers/', 60, NULL FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM dashboard_apps h WHERE h.url LIKE '/views/flowers%')
ON DUPLICATE KEY UPDATE dashboard_apps.name = dashboard_apps.name;
...
WHERE NOT EXISTS (SELECT 1 FROM dashboard_apps h WHERE h.url LIKE '/views/maze%')
ON DUPLICATE KEY UPDATE dashboard_apps.name = dashboard_apps.name;

INSERT INTO dashboard_apps (name, icon, gradient, url, sort_order, project_id)
SELECT 'Backing Tracks', 'fas fa-music', 'linear-gradient(45deg, #667eea 0%, #56ccf2 100%)', '/views/music/', 100, NULL FROM DUAL
WHERE NOT EXISTS (SELECT 1 FROM dashboard_apps h WHERE h.url LIKE '/views/music%')
ON DUPLICATE KEY UPDATE dashboard_apps.name = dashboard_apps.name;
...
SELECT u.id, h.id
FROM users u
JOIN dashboard_apps h ON h.is_default = 1
WHERE u.is_active = 1
